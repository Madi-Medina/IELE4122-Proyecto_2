...
% Caso Eólico: 23 generadores síncronos + 9 parques eólicos
...
% gen_FNCER: 1 = síncrono, 0 = FNCER (eólico)
% 32 generadores total: 23 síncronos + 9 eólicos
...
% Parques 5-8: 155 MW (Buses 15, 16, 23)
% Parque 9: 350 MW (Bus 23, zona Sur)
Sn = [76 76 76 76 155 155 155 155 350];

% Parámetros Weibull: [a_dia, b_dia, a_noche, b_noche]
data_eolica_norte = [12 1.85 12 1.85];
data_eolica_sur = [10 1.75 10 1.75];

% Turbina eólica
Sn_turbina = 2.750; % Capacidad nominal [MW]
c1 = 1:1:30;
...
[MC1, Sn_FNCER1] = Generacion_eolica(data_eolica_norte, Sn(1)*factor_cap, Sn_turbina, info_Turbina, FP_norte);
[MC2, Sn_FNCER2] = Generacion_eolica(data_eolica_norte, Sn(2)*factor_cap, Sn_turbina, info_Turbina, FP_norte);
[MC3, Sn_FNCER3] = Generacion_eolica(data_eolica_norte, Sn(3)*factor_cap, Sn_turbina, info_Turbina, FP_norte);
[MC4, Sn_FNCER4] = Generacion_eolica(data_eolica_norte, Sn(4)*factor_cap, Sn_turbina, info_Turbina, FP_norte);
[MC5, Sn_FNCER5] = Generacion_eolica(data_eolica_sur, Sn(5)*factor_cap, Sn_turbina, info_Turbina, FP_sur);
[MC6, Sn_FNCER6] = Generacion_eolica(data_eolica_sur, Sn(6)*factor_cap, Sn_turbina, info_Turbina, FP_sur);
[MC7, Sn_FNCER7] = Generacion_eolica(data_eolica_sur, Sn(7)*factor_cap, Sn_turbina, info_Turbina, FP_sur);
[MC8, Sn_FNCER8] = Generacion_eolica(data_eolica_sur, Sn(8)*factor_cap, Sn_turbina, info_Turbina, FP_sur);
[MC9, Sn_FNCER9] = Generacion_eolica(data_eolica_sur, Sn(9)*factor_cap, Sn_turbina, info_Turbina, FP_sur);
...
fprintf('CASO EÓLICO - %s (factor_cap = %d)\n\n', periodo_str, factor_cap);

fprintf('  Sistema:\n');
fprintf('    Generadores: %d síncronos + %d eólicos\n', num_gen_sinc, sum(gen_FNCER == 0));
...
case_name_nivel1 = sprintf('N1_Eolica_%s_d%d_f%d_k%s', periodo_str, p_max, factor_cap, k_str);
...
case_name_nivel2 = sprintf('N2_Eolica_%s_d%d_f%d_k%s', periodo_str, p_max, factor_cap, k_str);
